...
<h3 class="text-center">Pisos</h3>
    <div class="row form-inline">
        <a href="<?=base_url('pisos/')?>nuevo" class="btn btn-info btn-sm" role="button" >Nuevo</a>
      ...
      <table class="table w-auto small" >
        ...
        <tbody>
          ...
          <tr>
            ...
            <td><?= $item->SE_FECMOD ?></td>
            <td><?= $item->SE_ESTADO == 1 ? 'activo' : 'inactivo' ?></td>

            <td >
                <a class="btn btn-warning btn-sm" href="<?=base_url('pisos/editar/'.$item->SE_ID)?>" role="button">Editar</a> / <a class="btn btn-danger btn-sm" href="#" data-id="<?=$item->SE_ID?>" id="delete" role="button">Eliminar</a>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
