$("#mainsearch").on('keyup', function () {
  ...
  texto  =$('#mainsearch').val()  ; 
  
  $.ajax({
    type: 'ajax',
      url : '/codeigniter3/bien/temporal/'+texto,
      ...
      success:function(data) {
        ...
        for(i=0; i<data.length; i++){
          ...
          html +=  '<tr   id="'+data[i].Codigo+'">'+
              '<td>'+data[i].Codigo + '</td>'+
              '<td>'+data[i].Ficha + '</td>'+
              '<td>'+data[i].Codigo_Barra +'</td>'+
              '<td>'+data[i].Descripcion +'</td>'+
              '<td>'+data[i].Componente_de +'</td>'+
              '<td>'+data[i].Codigo_Principal +'</td>'+
              '<td>'+data[i].Modelo +'</td>'+
              '<td>'+data[i].Serie +'</td>'+
              '<td><a class="btn btn-warning btn-sm" href="' + window.location.pathname +'/editar/'+ data[i].Id + '" role="button">Editar</a> <a class="btn btn-danger btn-sm" href="#" data-id=' +  data[i].Id +  ' id="delete" role="button">Eliminar</a></td>'+ 
              '</tr>';

   
       
        }
        $("#listbienesbusqueda").html(html);
      },
  ...
  function cargarmejoras(){
    var b =$('input[name="AC_IDACTIVO"]').val()  ;
    texto=b;
   
      $.ajax({
        type: 'ajax',
          url : '/codeigniter3/bien/get_mejoras_data/'+texto,
          ...
          success:function(data) {
            console.log( data);
              ...
              for(i=0; i<data.length; i++){
                ...
                html +=  '<tr >'+
                    '<td>'+data[i]['Tipo.Documento'] + '</td>'+
                    '<td>'+data[i]['Serie.Doc.']+'</td>'+
                    '<td>'+data[i]['Numero.Documento']+'</td>'+
                    '<td>'+data[i]['Fecha.Mejora']+'</td>'+
                    '<td>'+data[i]['Moneda.Mejora']+'</td>'+
                    '<td>'+data[i]['Monto.Mejora']+'</td>'+
                    '<td>'+data[i]['T.Cambio.Mejora']+'</td>'+
                    '<td>'+data[i]['Observaciones']+'</td>'+
                    '</tr>';
              }
            $('#listmejoras').html(html);
      ...
      function cargarajustes(){
        ...
        texto=b;
          ...
          $.ajax({
            type: 'ajax',
              url : '/codeigniter3/bien/get_ajustes_data/'+texto,
                ...
                console.log( data);
                  ...
                  for(i=0; i<data.length; i++){
                    ...
                    html +=  '<tr >'+
                        ...
                        '<td>'+data[i]['CODIGO']+'</td>'+
                        '<td>'+data[i]['DESCRIPCION']+'</td>'+
                        '<td>'+data[i]['VALOR_LIBRO']+'</td>'+
                        '<td>'+data[i]['OPT_AJUSTE_LIBRO']+'</td>'+
                        '<td>'+data[i]['AJUSTE_LIBRO']+'</td>'+
                        '<td>'+data[i]['OPT_AJUSTE_TRIBUTARIO']+'</td>'+
                        '<td>'+data[i]['AJUSTE_TRIBUTARIO']+'</td>'+
                        '<td>'+data[i]['OPT_AJUSTE_FINANCIERO']+'</td>'+
                        '<td>'+data[i]['AJUSTE_FINANCIERO']+'</td>'+
                        '<td>'+data[i]['CUENTA_CONTABLE']+'</td>'+
                        '</tr>';
                  }
  ...
  $('input[name="AC_VH_SOLES"]').on('keyup', function () {
            ...
            texto  =$('input[name="AC_VH_SOLES"]').val()  ; 
             ...
             // $('input[name="AC_VALOR_LIBRO_CONTABLE"]').val(texto);
              alert($('input[name="AC_VALOR_LIBRO_CONTABLE"]').val(texto));
           //   alert( $('input[name="AC_VALOR_LIBRO_CONTABLE"]').val(texto));
          //    if($('input[name="AC_VALOR_LIBRO_CONTABLE"]').val()= ""){
           //     $('input[name="AC_VALOR_LIBRO_CONTABLE"]').val("0,00");
            //  }else{
            //    $('input[name="AC_VALOR_LIBRO_CONTABLE"]').val(texto)*1.0;
